pipeline {
  ...
  stage('Build & Run Container') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'docker-login', passwordVariable: 'REGISTRY_PASSWD', usernameVariable: 'name')]) {
                ...
                //withCredentials([usernamePassword(credentialsId: 'docker-login', usernameVariable: 'insta7120', passwordVariable: 'dock-hub')]) {
                    sh "docker login -u $name -p $REGISTRY_PASSWD"
